"use client"
import { removePhone } from '@/actions/phone/phone-remove';
...
const RemoveOverlay = ({ param, handleClose }: props) => {
    ...
    const [state, formAction] = useFormState<any, FormData>(removePhone, undefined);
    HandleCloseModal(state, handleClose)
    
    return (
        <form className='flex flex-col gap-3 text-center' action={formAction}>
            <RemoveOverlayContent id={param.id} modelName='phone' handleClose={handleSubmit} />
        </form>
    )
}
